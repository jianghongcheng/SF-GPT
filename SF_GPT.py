...
class SF_GPG(nn.Module):
    ...
    def forward(self, in_pix, in_dct):
        ...
        fused = self.SF_GPG(out_pix, out_dct)
        return fused, out_dct
...
class SF_GPT(nn.Module):
    ...
    def forward(self, x):
        dct_input = self.dwt(x)                       # [B, 4, 128, 128]
        x_pix = self.head1(dct_input)                # [B, 128, 128, 128]
        x_dct = self.head2(dct_input)                # [B, 128, 128, 128]
