...
root1 = path1 = '/media/max/a/DCT2021/Semantic-Colorization-GAN-main_HC (H) (tr)/train/dataset/rgb/'  # 网络输出后的图像##11 is best
root2 = path2 = 'output/test_T/'  # gt
...
# path3.sort(key=lambda x: int(x.replace("validation_","").split('.')[0]))#按照数字进行排序后按顺序读取文件夹下的图片
...
result = [[0 for _ in range(255)] for _ in range(255)]

print(len(result[0]))
##################################
# img_c = np.zeros((256,256))
# for i in range(f_nums):#从0开始
...
#
# img_d = img_c/372
# np.save("mean_t.npy", img_d)
###########################################
img_c = np.zeros((256,256,3))
for i in range(f_nums):#从0开始
    img_a = cv2.imread(root1 + path1[i])/255
    img_b = img_a
    print(img_a[:,:,1].shape)
