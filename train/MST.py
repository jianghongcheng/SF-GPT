...
class MS_MSA(nn.Module):
    def __init__(
        ...
        self.to_v = nn.Linear(dim_in * 2, dim_head * heads, bias=False)
    ...
    def forward(self, x_pix, x_dct):
        ...
        q_inp = self.to_q(x)
        k_inp = self.to_k(x)
        x_v = torch.cat((x_pix,x_dct),dim= 3)
        x_v = x_v.reshape(b,h*w,2*c)
        v_inp = self.to_v(x_v)
...
######################################################################################
        ...
        out_c = self.proj(x).view(b, h, w, self.dim_out) #torch.Size([2, 128, 128, 128])

        # print(out_c.shape)
        #out_p = self.pos_emb(v_inp.reshape(b,h,w,c).permute(0, 3, 1, 2)).permute(0, 2, 3, 1)
        # print(v_inp.reshape(b,h,w,self.dim_head).permute(0, 3, 1, 2).shape)
        out_p = self.pos_emb(v_inp.reshape(b,h,w,self.dim_head).permute(0, 3, 1, 2)).permute(0, 2, 3, 1)
        out = out_c + out_p
    ...
    def __init__(
        ...
        for _ in range(num_blocks):
            self.blocks.append(nn.ModuleList([
                MS_MSA(dim_in=dim_in // 2, dim_head=dim_head, dim_out=dim_out, heads=heads),
